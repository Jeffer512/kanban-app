...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Register failed. Please check your credentials.');
    }
  };
    ...
    <div className="flex min-h-screen items-center justify-center bg-app-bg p-4">
      ...
      <div className="w-full max-w-md rounded-2xl bg-card-bg p-8 shadow-2xl border border-border transition-colors duration-300">
        ...
        <p className="mt-8 text-center text-sm text-text-muted">
          Already have an account? <Link to="/login" className="font-bold text-text-main hover:underline">Login</Link>
        </p>
      </div>
    </div>
  );
